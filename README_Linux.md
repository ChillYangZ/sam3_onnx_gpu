...
```bash
sudo apt-get update
sudo apt-get install -y python3-pip curl zip
python3 -m pip install -r requirements-api.txt
```

以上系统依赖安装命令适用于 Ubuntu/Debian；其他 Linux 发行版请使用对应的软件包管理器安装 `python3-pip`、`curl` 和 `zip`。
...
curl -L --fail --retry 3 -C - \
  -o "onnx-models/text-encoder.onnx" \
...
curl -L --fail --retry 3 -C - \
  -o "onnx-models/text-encoder-fp16.onnx" \
...
```bash
python3 -m uvicorn api_server:app --host 0.0.0.0 --port 8510
```
...
model directory、tokenizer path、output directory、model precision和结果保存开关固定配置在 `api_server.py` 中，不会读取通过 `export` 设置的环境变量。如需调整，请直接修改以下常量：
...
```bash
python3 -m uvicorn api_server:app --host 0.0.0.0 --port 8510 --workers 1
```
...
```bash
curl -X POST "http://127.0.0.1:8510/predict" \
...
以下所有多行 curl 示例均使用 Linux shell 的 `\` 作为换行符。`\` 必须是该行最后一个字符，后面不能有空格。
...
以下命令适用于 Bash 等常见 Linux shell，也可以将每条 curl 指令写在同一行。
...
不要在请求中传递类似 `input_dir=/home/user/images` 的客户端本地路径。项目部署后，该路径会被解释为服务器上的路径，而不是运行 curl 的客户端路径。
